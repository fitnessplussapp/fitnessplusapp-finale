...
const getWeekDays = (baseDate: Date) => {
  ...
  // Pazar(0) ise -6, değilse 1-day kadar geriye git (Pazartesi için)
...
const WeeklySchedule: React.FC = () => {
  ...
  const handleAddParticipant = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (participantType === 'existing') {
        ...
        // Zaten bu eventte var mı kontrolü
        if (selectedEvent.participants.some(p => p.memberId === member.id)) throw new Error("Bu üye zaten BU derse ekli.");
      ...
      fetchAllData();
    } catch (err: any) { alert(err.message); } finally { setProcessLoading(false); }
  };
  ...
  const handleConfirmRemoval = async (shouldRefund: boolean) => {
    ...
    try {
      ...
      fetchAllData();
    } catch (error) { console.error(error); alert("Hata oluştu."); } finally { setProcessLoading(false); }
  };
  ...
  return (
    <div className={styles.schedulePage}>
      ...
      <div className={styles.headerContainer}>
        <div className={styles.headerTop}>
          <div className={styles.titleGroup}>
            <Link to={`/admin/coaches/${coachId}`} className={styles.backLink}><ArrowLeft size={14}/> Koç Listesine Dön</Link>
               ...
               <div key={hour} className={styles.timeSlot}>
                 <div className={styles.slotHeader}>
                    ...
                    <button 
                      ...
                      title="Bu saate yeni etkinlik ekle"
                    >
                      <Plus size={16} />
                    </button>
                 </div>
      ...
      {/* --- MODAL 1: YENİ ETKİNLİK (GÜNCELLENDİ) --- */}
      <Modal isOpen={isEventModalOpen} onClose={() => setIsEventModalOpen(false)} title="Program Oluştur">
        <form onSubmit={handleCreateEvent} className={styles.modalForm}>
          
          {/* Tablar (Segmented Control) */}
          <div className={styles.segmentControl}>
            <button 
              type="button" 
              className={`${styles.segmentBtn} ${eventType === 'personal' ? styles.segmentBtnActive : ''}`} 
            ...
            <button 
              type="button" 
              className={`${styles.segmentBtn} ${eventType === 'group' ? styles.segmentBtnActive : ''}`} 
          ...
          {eventType === 'personal' ? (
            <div className={styles.inputGroup}>
              <label className={styles.inputLabel}>Üye Seç</label>
                ...
                >
                  ...
                  {members.map(m => (
                    <option key={m.id} value={m.id} disabled={m.currentSessionCount <= 0}>
                      {m.name} (Kalan: {m.currentSessionCount})
                    </option>
                  ))}
                </select>
            ...
            <>
              ...
              <div className={styles.inputGroup}>
                <label className={styles.inputLabel}>Kota</label>
                <input 
                  type="number" 
                  className={styles.customInput} 
                  value={eventQuota} 
          ...
          <div className={styles.formActions}>
            <button type="button" className={`${styles.actionBtn} ${styles.btnSecondary}`} onClick={() => setIsEventModalOpen(false)}>İptal</button>
            <button type="submit" className={`${styles.actionBtn} ${styles.btnPrimary}`} disabled={processLoading}>
               {processLoading ? <Loader2 className={styles.spinner} size={20}/> : 'Kaydet'}
            </button>
          </div>
        </form>
      </Modal>

      {/* --- MODAL 2: DETAY (AYNI MANTIK) --- */}
      <Modal isOpen={isDetailModalOpen} onClose={() => setIsDetailModalOpen(false)} title="Etkinlik Detayları">
        {selectedEvent && (
          <div className={styles.detailContainer}>
            ...
            <div className={styles.eventInfoBox}>
              ...
              <div className={styles.infoMeta}>
                ...
                <span>{selectedEvent.participants.length} / {selectedEvent.quota} Katılımcı</span>
              </div>
            </div>
            ...
            <div className={styles.participantList}>
              {selectedEvent.participants.length === 0 && <p className={styles.noData}>Henüz katılımcı yok.</p>}
            ...
            <button className={styles.deleteEventBtn} onClick={initiateDeleteEvent}><Trash2 size={16}/> Etkinliği Sil</button>
          </div>
        )}
      </Modal>

      {/* --- MODAL 3: KATILIMCI EKLE (GÜNCELLENDİ) --- */}
      <Modal isOpen={isParticipantModalOpen} onClose={() => setIsParticipantModalOpen(false)} title="Kişi Ekle">
        <form onSubmit={handleAddParticipant} className={styles.modalForm}>
           <div className={styles.segmentControl}>
              <button type="button" className={`${styles.segmentBtn} ${participantType === 'existing' ? styles.segmentBtnActive : ''}`} onClick={() => setParticipantType('existing')}>Mevcut Üye</button>
           ...
           {participantType === 'existing' ? (
             <div className={styles.inputGroup}>
                <label className={styles.inputLabel}>Üye</label>
                <div className={styles.selectWrapper}>
           ...
           <div className={styles.formActions}>
             <button type="submit" className={`${styles.actionBtn} ${styles.btnPrimary}`} disabled={processLoading}>Ekle</button>
           </div>
        </form>
      </Modal>

      {/* --- MODAL 4: İADE ONAYI (AYNI) --- */}
      <Modal isOpen={removalState.isOpen} onClose={() => setRemovalState({isOpen:false, target:null})} title="Silme İşlemi">
        <div className={styles.confirmBody}>
          <div className={styles.confirmIconContainer}>
             <AlertTriangle size={36} color="#f59e0b" />
          </div>
          <p className={styles.confirmText}>{removalState.target === 'event' ? "Tüm etkinlik silinecek. Üyelerin hakları iade edilsin mi?" : "Kişi çıkarılacak. Hakkı iade edilsin mi?"}</p>
          <div className={styles.refundActions}>
            ...
            <button className={`${styles.refundButton} ${styles.noRefund}`} onClick={() => handleConfirmRemoval(false)} disabled={processLoading}>
              <Ban size={18} className={styles.dangerIcon}/> <div><span className={styles.btnTitle}>Hayır, İade Etme</span><span className={styles.btnDesc}>Hakkı yanar.</span></div>
            </button>
          </div>
        </div>
      </Modal>
